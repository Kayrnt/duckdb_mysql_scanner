...
/*fn duckdb_type_2(type_info: &MysqlTypeInfo) -> LogicalType {
    ...
    if mysql_type_name == "enum" {
        ...
        return LogicalType::Enum(
            format!("mysql_enum_{}", mysql_type_name),
            duckdb_levels,
            enum_values.len(),
        );
    }

    match mysql_type_name.as_str() {
        "bit" => LogicalType::Bit,
        ...
        "decimal" => LogicalType::Decimal(type_info.numeric_precision, type_info.numeric_scale),
        "char" | "bpchar" | "varchar" | "text" | "jsonb" | "json" => LogicalType::VarChar,
        ...
        _ => LogicalType::Invalid,
    }
}

fn duckdb_type(info: &MysqlColumnInfo) -> LogicalType {
